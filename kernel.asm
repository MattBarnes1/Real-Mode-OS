...
;kernel.asm contains assembly functions that you can use in your kernel
	...
	.global _Out
	.global _In
	.global _ExtInterrupt



;char readFromMemory(segment, address, struct)
_readFromMemory:
	push bp
	mov bp,sp
	push ds
	mov bx,[bp+4] ; our segment
	mov si,[bp+6] ; our address
	mov dx,[bp+8] ; our struct
	mov ds,bx ;
	mov ax,[si]
	xor ah, ah ; clear ah since we're only returning the character
	pop ds
	pop bp
	ret
...
;void writeToPort(int Port, char myChar)
_Out:
	push bp
	mov bp,sp
	mov dx, [bp+4]
...
;unsigned char readFromPort(char port)
_In:
	push bp
	mov bp,sp
	mov dx, [bp+4]
...
;int ExtInterrupt (int number, int AX, int BX, int CX, int DX, int ES)
_ExtInterrupt:
	push bp
	mov bp,sp
	mov ax,[bp+4]	;get the interrupt number in AL
	...
	mov di,[bp+14]
	mov dx, cs
	mov es, dx; so es;di == structure
	pop dx
ext:	int #0x00	;call the interrupt (00 will be changed above)
